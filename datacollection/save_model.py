...
def plot_significant_acorr(res, ax, is_acf = True):
    ...
    else: resid, conf_int = pacf(res.resid[1:].dropna(), nlags = 300, alpha = 0.005)
    ...
    ax.vlines(resid.index[significant == True], 
              ...
              ymax = resid[significant == True], linewidth = 0.5, color = lavender);
    ...
    for i in significant[significant == True].index:
        ...
        one_unit = width / 100
        if resid[i] < 0: ax.text(x = i - len(str(i)) * 1.4, y = resid[i] - 2.75 * one_unit, s = f"{round(i / 12, 1)}h")
        else: ax.text(x = i - len(str(i)) * 1.1, y = resid[i] + 2 * one_unit, s = f"{round(i / 12, 1)}h")
        ax.set_ylim(
...
def plot_and_save(res, df):
    ...
    axs[0, 2].plot(res.autoregressive["smoothed"], color = lavender)
    ...
    axs[2, 2].plot(fore.predicted_mean, color = lavender, linewidth = 0.5)
